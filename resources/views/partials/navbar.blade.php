...
<div class="app-navbar flex-grow-1 justify-content-end" id="kt_app_header_navbar">
    <div class="app-navbar-item ms-2 ms-lg-6">
        <div class="cursor-pointer symbol symbol-circle symbol-35px symbol-lg-45px" data-kt-menu-trigger="{default: 'click', lg: 'hover'}"
            data-kt-menu-attach="parent" data-kt-menu-placement="bottom-end">
            @if ($user->profile_photo_path || $user->google_avatar_url)
                <img src="{{ $user->avatarUrl() }}" alt="{{ $user->name }}" class="symbol-label" style="object-fit: cover;">
            @else
                <div class="symbol-label bg-primary text-white fw-bold">{{ $user->initials() }}</div>
            @endif
        </div>
            ...
            <div class="menu-item px-3">
                <div class="menu-content d-flex align-items-center px-3">
                    <div class="symbol symbol-50px me-5">
                        @if ($user->profile_photo_path || $user->google_avatar_url)
                            <img src="{{ $user->avatarUrl() }}" alt="{{ $user->name }}" class="symbol-label"
